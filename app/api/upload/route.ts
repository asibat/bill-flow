...
export async function POST(request: NextRequest) {
  ...
  if (mimeType.includes('pdf')) {
    extractionResponse = await extractFromDocument(base64, 'application/pdf', user.id, { explanationLanguage })
  } else {
    const validMime = mimeType.includes('png') ? 'image/png' :
                      mimeType.includes('webp') ? 'image/webp' : 'image/jpeg'
    extractionResponse = await extractFromImage(base64, validMime, user.id, { explanationLanguage })
  }
